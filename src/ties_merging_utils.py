...
def topk_values_mask(M, K=0.7, return_mask=False, flat_ptm=1, reverse=False):
    if K > 1:
        K /= 100
    ...
    kth_values, _ = (M/flat_ptm).abs().kthvalue(k, dim=1, keepdim=True)
    # Create a mask tensor with True for the top k elements in each row
    if reverse:
        mask = (M/flat_ptm).abs() <= kth_values
    else:
        mask = (M/flat_ptm).abs() >= kth_values
    if original_shape == M.squeeze().shape:
        final_mask = mask.squeeze()
        M = M.squeeze()
        out_mask = final_mask.float()
    else:
        final_mask = mask
        out_mask = final_mask.float().mean(dim=1)

    if return_mask:
        return M * final_mask, final_mask.float().mean(dim=1), final_mask
    return M * final_mask, out_mask
