...
def get_dataloader(dataset, is_train, args, image_encoder=None, use_val=False, val_test=False):
    if image_encoder is not None:
        feature_dataset = FeatureDataset(is_train, image_encoder, dataset, args.device)
        dataloader = DataLoader(feature_dataset, batch_size=args.batch_size, shuffle=is_train, num_workers=14)
    else:
        if use_val:
            dataloader = dataset.val_loader if not val_test else dataset.test_loader
        else:
